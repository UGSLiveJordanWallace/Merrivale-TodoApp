...
export async function update(id, is_complete, task, list) {
    ...
    if (error2) {
        return { error: "Something Went Wrong", error_message: error2 };
    }

    if (is_complete) {
        await supabase
            .from("logs")
            .insert({ task: task, email: user.user.email, list: list });
    }
